...
class PlayersController extends AppController {
   ...
   public function delete($id = null)
         ...
         if ($type == 'number')
         {
            ...
            foreach ($players as $p)
            {
            	$this->Player->id = $p['Player']['id'];
               	$this->Player->saveField('tryout_number', $number++);
            }
            $players = $this->Player->find('all', compact('conditions', 'order'));
         }
	...
	function allstar_tryout($type=null, $league_id=0)
	{
      ...
      if ($league_id > 0)
      {
         ...
         if ($type == 'number')
         {
            ...
            {
            	$this->Player->id = $p['Player']['id'];
                $this->Player->saveField('allstar_tryout_number', $number++);
            }
            $players = $this->Player->find('all', compact('conditions', 'order'));
         }
         $this->set('league_id', $league_id);
	 ...
	 * List of players for tryout
	 */
	function accelerated_tryout($type=null, $league_id=0)
	{
	   if (empty($league_id))
	   {
	      $league_id = $this->Auth->user('default_league_id');
      ...
      if ($league_id > 0)
      {
         $conditions = array('Player.league_id'=> $league_id, 'Player.accelerated_tryout'=> 1);
         $order = array('last_name DESC', 'first_name DESC');
         $players = $this->Player->find('all', compact('conditions', 'order'));
         if ($type == 'number')
         {
            ...
            {
               $this->Player->id = $p['Player']['id'];
               $this->Player->saveField('accelerated_tryout_number', $number++);
            }
            $players = $this->Player->find('all', compact('conditions', 'order'));
         }
         $this->set('league_id', $league_id);
      ...
      {
         // CSV file
         $this->set('title_for_layout', 'AcceleratedTryoutList');
         $this->layout = 'csv';
         $this->render('accelerated_tryout_csv');
      }
      else if ($type == 'pdf')
      {
         // PDF file
         $this->set('title_for_layout', 'AcceleratedTryoutList.pdf');
         $this->layout='pdf';
         $this->render('accelerated_tryout_pdf');
      }
      else
      {
        $this->loadModel('League');
